...
/**
 * The Robot class simulates the movements and sensor readings of a robot
 * navigating a maze.
 * It maintains the current position of the robot, the direction it is facing,
 * and the layout of the maze.
 * The robot can move forward, move backward (by turning and moving forward),
 * and rotate left or right.
 * It uses a simple character-based representation for the maze ('O' for open,
 * 'W' for wall, and 'G' for goal).
 */
public class Robot {
    ...
    /**
     * Initializes the Robot within the maze and sets its position at the 'R'
     * character.
     *
     ...
     * @param x
     */
    public Robot(char[][] maze, int x, int y, String Init_direction) {
        this.maze = maze;
     ...
     * The robot can only move if the space is open, and it updates the maze to
     * reflect the new position.
     *
     * @return true if the robot moves forward; false if it hits a wall or the edge
     *         of the maze.
     */
    public boolean moveForward() {
        ...
        switch (facingDirection) {
            ...
            case LEFT:
                ...
                break;
            case RIGHT:
                if (y < maze[0].length - 1 && maze[x][y + 1] != 'W') {
                    // maze[x][y] = 'O';
                    y++;
                    // maze[x][y] = 'R';
                    return true;
    ...
    /**
     * Moves the robot backward by first changing its facing direction to the
     * opposite
    ...
    public boolean moveBackward() {
        // Store the direction
        Direction trueDirection = facingDirection;
        // Change facing direction to the opposite and attempt to move forward.
        switch (facingDirection) {
            // instead of doing the whole process, change the facing direction to oppsite
            // and call moveForward
            case UP:
        ...
        if (moveForward()) {
            // after move restore the direction back to the original direction
            facingDirection = trueDirection;
            return true;
        }
        facingDirection = trueDirection;
        return false;
    ...
    /**
     * Checks the sensor in the given direction relative to the robot's current
     * position
     * and returns the state of the cell in that direction.
     *
     * @param directionStr The direction to check, expressed as a string
     *                     corresponding to the Direction enum.
     * @return The CellState (WALL, OPEN, or GOAL) of the cell in the specified
     *         direction.
     */
    public CellState checkSensor(String directionStr) {
        // Convert the string to a Direction enum and check the cell state in that
        // direction.
        Direction direction = Direction.valueOf(directionStr);
        switch (direction) {
            case UP:
                if (x > 0)
                    return getCellState(maze[x - 1][y]);
                break;
            case DOWN:
                if (x < maze.length - 1)
                    return getCellState(maze[x + 1][y]);
            ...
            case LEFT:
                if (y > 0)
                    return getCellState(maze[x][y - 1]);
                break;
            case RIGHT:
    ...
    /**
     * Helper method to determine the state of a cell given its character
     * representation.
     *
